...
setlocal
chcp 65001 >nul
powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-Content -Encoding UTF8 '%~f0' | Out-String | Invoke-Expression"
exit /b
#>
...
$jsonArray = $currentResults | ConvertTo-Json -Compress
$historyJson = $historyRows | ConvertTo-Json -Compress
...
function historyFor(id) {
  return HISTORY
    .filter((row) => String(row.ID).replace("\\\\", "") === id)
...
function estimateDays(drive, rows) {
  const samples = rows.slice(-8);
  if (samples.length < 2) return "样本不足";
  const first = samples[0];
  const last = samples[samples.length - 1];
  const hours = (new Date(last.Timestamp) - new Date(first.Timestamp)) / 36e5;
  const growth = (Number(last.Used) || 0) - (Number(first.Used) || 0);
  if (hours <= 0 || growth <= 0.05) return "暂无增长压力";
  const dailyGrowth = growth / Math.max(hours / 24, 1 / 24);
...
function renderOverview() {
  ...
  $("insights").innerHTML = [
    ...
    insight("增长最快", fastest ? `${fastest.id} 本次 ${fastest.diff >= 0 ? "增加" : "减少"} ${fmt(Math.abs(fastest.diff))}` : "-", fastest?.diff > 0 ? "warn" : "good"),
    ...
    return `
      <article class="card ${d.status}">
        <div class="card-top">
          ...
          <div class="badge">${statusText(d.status)} ${pct(d.percent)}</div>
        </div>
...
[System.IO.File]::WriteAllText((Resolve-Path ".").Path + "\$htmlFile", $html, $utf8Bom)
